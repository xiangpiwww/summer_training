    ...
    <div>
        ...
        <div class="ct">
            ...
            <el-checkbox-group v-model="checkedFlies" @change="handleCheckedChange">
                <div v-for="file in fileSearchList"  :key="file.id">
                    <el-checkbox  :label="file.id">{{ file.filename }}</el-checkbox>
                    <div v-for="data in file.content" :key="data.line">
                        <span>{{ data.line }}</span>
                        <span>{{ data.text }}</span>
                    </div>
                </div>
    ...
    export default {
        ...
        methods:{
            ...
            submitButton:async function(){
                try{
                    // 等待后端的返回的端口已经方式
                    const response = await axiox.get("http://localhost:8888/getDir?dir="+this.input+"&&key="+this.searchkey)
                    currentArray = response.data
                    // 从后端获取返回的数据
